...
export default function BuySellPage() {
  ...
  async function submit() {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/orders', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ amount: Number(amount), ...fields }) })
    ...
    <main className="min-h-dvh relative">
      <div className="mx-auto max-w-xl px-5 pt-10 pb-24">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4">
          <div><label className="label">Фамилия*</label><input className="input" value={fields.lastName} onChange={e=>setFields(v=>({...v,lastName:e.target.value}))} /></div>
          <div><label className="label">Имя*</label><input className="input" value={fields.firstName} onChange={e=>setFields(v=>({...v,firstName:e.target.value}))} /></div>
          <div><label className="label">Отчество*</label><input className="input" value={fields.middleName} onChange={e=>setFields(v=>({...v,middleName:e.target.value}))} /></div>
          <div><label className="label">Адрес кошелька TRC-20 (34 символа)*</label><input className="input" placeholder="T..." value={fields.trc20} onChange={e=>setFields(v=>({...v,trc20:e.target.value}))} /></div>
        </div>
        ...
        <div className="mt-6 flex gap-3"><button onClick={submit} disabled={loading} className="btn btn-primary">{loading? 'Создание...' : 'Создать заявку'}</button></div>
        {lastId && (
