...
export default function AMLPage() {
  const [addr, setAddr] = useState(''); const [left, setLeft] = useState(5)
  ...
  async function check() {
    ...
    const r = await fetch('/api/aml', { method:'POST' })
    setRes(await r.json()); setLeft(v=>v-1)
  }
  return (
    <main className="min-h-dvh relative">
      <div className="mx-auto max-w-xl px-5 pt-10 pb-24">
        ...
        {res && (<div className="card p-5 mt-6"><div className="text-lg font-semibold">Результат</div><div className="text-white/80 mt-2">Риск‑скор: {res.score}</div><div className="text-white/80">Вердикт: {res.verdict}</div></div>)}
      </div>
    </main>
  )
}